...
router.post("/login", validate(userLoginSchema), async (req, res) => {
  try {
    ...
    return res.status(200).header("x-auth-token", token).send(_.pick(user, ["_id", "name", "image", "rating", "ratingCount", "gender", "phone", "role"]));
...
router.put("/edit/:id", auth, validate(userUpdateSchema), async (req, res) => {
  try {
    ...
    if(req.user._id !== id)  return res.status(403).send("Forbidden. You can only edit your own profile.");
    ...
    return res
      .status(200)
      .send(_.pick(updatedUser, ["name", "email", "phone", "image", "_id"]));
