...
router.post("/post/:id", [auth, validate(createReport)], async (req, res) => {
  try {
    ...
    const existingReport = await ReportModel.findOne({reporter: req.user._id, reportedPost: postId})
    if(existingReport) return res.status(400).send("You already reported this post")
